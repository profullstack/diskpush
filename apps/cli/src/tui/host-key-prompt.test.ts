...
function tuiWithPrompt(onDecide: (trust: boolean) => void = () => {}) {
  ...
  ;(tui as unknown as { overlay: unknown }).overlay = {
    kind: 'hostKey',
    host: 'example.test',
...
const overlay = (tui: Tui) => (tui as unknown as { overlay: unknown }).overlay
...
describe('the host-key prompt', () => {
  ...
  it('still answers y and n, and keeps swallowing everything else', async () => {
    ...
    await expect(yes.onKey(key('y'))).resolves.toBe(true)
    expect(answers).toEqual([true])

    const no = tuiWithPrompt((trust) => answers.push(trust))
    await expect(no.onKey(key('n'))).resolves.toBe(true)
    expect(answers).toEqual([true, false])

    const esc = tuiWithPrompt((trust) => answers.push(trust))
    await expect(esc.onKey(key('escape'))).resolves.toBe(true)
    ...
    await expect(other.onKey(key('down'))).resolves.toBe(true)
    await expect(other.onKey(key('tab'))).resolves.toBe(true)
    expect(answers).toEqual([true, false, false])
    expect(overlay(other)).not.toBeNull()
  })
